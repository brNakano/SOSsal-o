    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (mode == "put"){
            if (intent.getSerializableExtra("schedule") is Schedule)
                schedulePut = intent.getSerializableExtra("people") as Schedule
            ...
            form_schedule_year.editText?.setText(ScheduleDate.year(schedulePut?.checkIn))
            form_schedule_hour.editText?.setText(ScheduleDate.hour(schedulePut?.checkIn))
            form_schedule_minute.editText?.setText(ScheduleDate.minute(schedulePut?.checkIn))
            var clientName = PeopleService.getPeopleById(this, schedulePut?.clientId)
            var employeeName = PeopleService.getPeopleById(this, schedulePut?.clientId)
